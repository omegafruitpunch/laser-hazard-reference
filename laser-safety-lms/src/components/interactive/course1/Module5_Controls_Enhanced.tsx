    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {hierarchyLevels.map((level, index) => (
          <div
            key={level.level}
            className={`${level.color} text-white rounded-lg transition-all cursor-pointer overflow-hidden`}
            style={{
...
const warningSigns = [
  { type: 'Class 2', text: 'Laser Radiation - Do Not Stare Into Beam', color: 'bg-amber-500' },
  { type: 'Class 3R/3B', text: 'Laser Radiation - Avoid Direct Eye Exposure', color: 'bg-orange-500' },
  { type: 'Class 4', text: 'Laser Radiation - Avoid Eye Or Skin Exposure', color: 'bg-red-600' },
  { type: 'Entry', text: 'Authorized Personnel Only - Laser In Operation', color: 'bg-red-700' },
];
...
function WarningSignsDemo() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`${warningSigns[selectedSign].color} text-white p-6 rounded-lg text-center`}>
        ...
        <Card key={index} className="p-3">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              {req.practical && (
                <div className="text-xs text-green-600 dark:text-green-400 mt-1">+ Practical</div>
              )}
            </div>
          </div>
        </Card>
...
const odRatings: ODRating[] = [
  { od: 1, attenuation: '10Ã— (90%)', use: 'Low power alignment' },
  { od: 2, attenuation: '100Ã— (99%)', use: 'Standard alignment' },
  { od: 3, attenuation: '1,000Ã— (99.9%)', use: 'Medium power work' },
  { od: 4, attenuation: '10,000Ã— (99.99%)', use: 'High power Class 4' },
  { od: 5, attenuation: '100,000Ã— (99.999%)', use: 'Very high power' },
  { od: 6, attenuation: '1,000,000Ã— (99.9999%)', use: 'Maximum protection' },
  { od: 7, attenuation: '10,000,000Ã— (99.99999%)', use: 'Ultrahigh power' },
];
...
function EyewearInspection() {
  ...
  return (
    <div className="space-y-3">
      ...
      {allChecked && (
        <div className="flex items-center gap-2 text-green-600 dark:text-green-400 text-sm font-medium p-2 bg-green-50 dark:bg-green-950/30 rounded-lg border border-green-200 dark:border-green-800">
...
function ImplementationChecklist() {
  ...
  return (
    <div className="space-y-4">
        ...
        {categories.map((category) => (
          <div key={category}>
            <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
              {category === 'Engineering' && <Settings className="w-4 h-4" />}
              {category === 'Administrative' && <FileText className="w-4 h-4" />}
              {category === 'PPE' && <HardHat className="w-4 h-4" />}
              {category}
            </h4>
      ...
      {progress === 100 && (
        <div className="flex items-center gap-2 text-green-600 text-sm font-medium p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
        ...
        {sortedData.map((item, index) => (
          <Card key={index} className="p-3">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              Maintenance: {item.maintenance} â€¢ Payback: {item.payback}
            </div>
          </Card>
        ))}
      </div>
...
function SectionNavigation({ activeSection, setActiveSection }: SectionNavigationProps) {
  const sections: { id: Section; label: string; icon: React.ReactNode }[] = [
    { id: 'hierarchy', label: 'Hierarchy', icon: <Shield className="w-4 h-4" /> },
    { id: 'engineering', label: 'Engineering', icon: <Building className="w-4 h-4" /> },
    { id: 'administrative', label: 'Administrative', icon: <FileText className="w-4 h-4" /> },
    { id: 'ppe', label: 'PPE', icon: <HardHat className="w-4 h-4" /> },
    { id: 'implementation', label: 'Implementation', icon: <Settings className="w-4 h-4" /> },
...
function SectionContent({ activeSection }: SectionContentProps) {
  switch (activeSection) {
    ...
    case 'engineering':
      return <EngineeringControlsSection />;
    case 'administrative':
      return (
        <div className="space-y-6">
          <SOPBuilder />
    ...
    >
      ...
      >
        <h1 className="text-2xl font-bold text-foreground flex items-center justify-center gap-3">
          ...
          Module 5: Laser Safety Controls
        </h1>
